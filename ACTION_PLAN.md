...
Continue Phase 2: migrate the remaining repositories (profile/plan/session) to SQLDelight following the sync-queue pattern (unblocks Workout History); add the sync-queue **drain workers** (WorkManager / BackgroundTasks) + a remote API to turn the sync stub into a live engine; offload DB calls to a background dispatcher (`kotlinx-coroutines-core`); optionally promote the Resume banner to a launch interstitial.
